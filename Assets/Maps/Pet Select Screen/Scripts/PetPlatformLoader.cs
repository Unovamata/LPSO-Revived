...
public class PetPlatformLoader : PetLoader{
    ...
    private void Start() {
        base.Start();
        selectedPet = petSpawnerMainDisplay.Find("PetSelected");
    ...
    private void LoadPetInPlatform(Transform platform, int i) {
        ...
        Pet petExtracted;
        SwitchSpritePetPlatform platformData = platform.GetComponent<SwitchSpritePetPlatform>();;
        ...
        //Creating the pet;
        Transform spawnerParent = platformData.petSpawner.transform;
        GameObject petInstance = Instantiate(pet, spawnerParent.position, Quaternion.identity, spawnerParent);
        DeactivateHourglass(platform);
