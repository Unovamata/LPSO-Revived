...
public class TextAnimations{
    ...
    public static void JumpAndFade(GameObject lean, Vector3 scale, float time) {
        ...
        try{
            ...
            LeanTween.value(lean, 1, 0, 1f).setOnUpdate((float val) => { text.opacity = val; }).setDelay(OpacityDelay(delayMove)); 
        } catch { }
    }
    ...
    public static void ResetTextParameters(GameObject lean, Vector3 scale) {
        ...
        TextMeshProUGUI text = lean.GetComponent<LPSOText>().whiteText.GetComponent<TextMeshProUGUI>();
